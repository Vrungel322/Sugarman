    ...
    private void openCamera() {
        ...
        if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA)
            ...
            try {
                ...
                File videoFolder = getFilesDir();

                videoFolder.mkdirs(); // <----
                //File video = new File(Const.FilesName.VIDEO_TEMP_FILE_NAME);


                String filename = Const.FilesName.VIDEO_TEMP_FILE_NAME;

                String mOriginalPath = Tools.getTempFolderPath() + "/" + filename;

                File video = new File(mOriginalPath);



                Uri uriSavedVideo = FileProvider.getUriForFile(RecordVideoActivity.this, RecordVideoActivity.this.getApplicationContext().getPackageName() + ".utils.GenericFileProvider", video);
