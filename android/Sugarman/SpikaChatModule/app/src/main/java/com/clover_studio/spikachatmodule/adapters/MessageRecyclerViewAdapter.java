    ...
    public class BaseViewHolder extends RecyclerView.ViewHolder implements Binder {
        ...
        public void bindItem(int position) {
            ...
            if (!isMessageFromUser(data.get(Math.max(0, position - 1)), message.user) || position == 0) {
                ...
                String str1 = message.user.name;
                str1 = str1.replaceAll("( +)"," ").trim();
                String name1 = str1;
if (str1.contains(" "))               name1 = str1.substring(0,(message.user.name.indexOf(" ")));
                name.setText(name1);
            }
            else {
            ...
            if (isMessageFromUser(message, myUser)) {
                if(message.seenBy != null && message.seenBy.size() > 0){
                    String text = message.getTimeCreated(time.getResources());
                    time.setText(text);
                    if(status!=null) {
                        status.setImageDrawable(time.getResources().getDrawable(R.drawable.seen_icon_big));
                        status.setVisibility(View.VISIBLE);
                    }
                } else if (message.status == Const.MessageStatus.SENT) {
                    time.setText(time.getResources().getString(R.string.sending___));

                    if(status!=null)status.setVisibility(View.INVISIBLE);
                    ...
                    if(status!=null) {
                        status.setImageDrawable(time.getResources().getDrawable(R.drawable.seen_lonely));
                        status.setVisibility(View.VISIBLE);
                    }
                }
