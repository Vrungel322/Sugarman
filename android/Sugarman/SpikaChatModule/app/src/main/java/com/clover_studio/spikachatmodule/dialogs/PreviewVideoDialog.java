...
public class PreviewVideoDialog extends Dialog {
  ...
  @Override public void show() {
    ...
    File videoFile =
        new File(Tools.getVideoFolderPath() + "/" + fileModel.file.name + "_" + fileModel.file.id);

    if (videoFile.exists()) {
      filePath = videoFile.getAbsolutePath();
    } else {

      mDialog = DownloadFileDialog.startDialog(getOwnerActivity());

      DownloadFileManager.downloadVideo(getOwnerActivity(),
          Tools.getFileUrlFromId(fileModel.file.id, getOwnerActivity()), videoFile,
          new DownloadFileManager.OnDownloadListener() {
            ...
            @Override public void onFinishDownload() {
              getOwnerActivity().runOnUiThread(new Runnable() {
                @Override public void run() {
                  mDialog.fileDownloaded();
                }
              });
            }

            @Override public void onResponse(boolean isSuccess, final String path) {
              if (!PreviewVideoDialog.this.isShowing()) { // or call isFinishing() if min sdk version < 17
                return;
              }
              getOwnerActivity().runOnUiThread(new Runnable() {
                @Override public void run() {
